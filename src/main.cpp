        ...
        if (gpu)
        {
            ...
            for (int i = 0; i < n_cells; i++)
            {
                ...
                GPUSPhase s_phase(
                    ...
                    std::stoi(arg_values[6]), has_dormant, data, arg_values[1],
                    context);
...
#pragma omp parallel for
            for (long long unsigned int i = 0; i < arg_values.cells; i++)
            {
                ...
                checkpoint_times.push_back(
                    std::pair<int, s_phase_checkpoints_t>(i,
                                                          s_phase->getTimes()));
            ...
            double sim_sum = 0; // Total microseconds spent in simulation phase
            double saved_sum = 0; // Total microseconds spent in saving phase
            ...
            std::cout << "[STAT] Average creation time   [ms] : " << created_avg
                      << std::endl;
            std::cout << "[STAT] Average simulation time [ms] : " << sim_avg
                      << std::endl;
            std::cout << "[STAT] Average saving time     [ms] : " << saved_avg
                      << std::endl;
